<template>
  <div class="BoxSix">
    <box title="企业分析" :active-tab="activeTab" :tab-content="tabContent" v-if="activeTab==1">
      <div class="box">
        <p>营收规模等级</p>
        <chart ref="bar1" :skey="'bar1'" :option="option1" class="chart"></chart>
      </div>
      <div class="line"></div>
      <div class="box">
        <p>税收规模等级</p>
        <chart ref="bar2" :skey="'bar2'" :option="option2" class="chart"></chart>
      </div>
    </box>
...
export default {
  ...
  data () {
    return {
      ...
      option3: {
        color: ['#0F2088', '#EFAA3C', '#D7087E', '#C879FF'],
        grid: {
          top: '5%',
          left: 0,
          right: 0,
      ...
      request.normalPort({
        ...
        res.data.data.resultList.map((item) => {
          ...
          if (item.code == 'Xh00031') {
            arr3.push(Number(item.value))
            arr3.push(Number(item.value))
          }
          if (item.code == 'Xh00032') {
            arr4.push(Number(item.value))
            arr4.push(Number(item.value))
          }
          time.push(item.time)
        })
